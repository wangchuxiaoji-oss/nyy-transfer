...
self.addEventListener("message", (event) => {
  ...
  if (data.type === "NYY_DEBUG_ENABLE" && event.source?.id) {
    ...
    postDebug("sw", "register", { id: data.id, fileName: data.file.fileName, fileSize: data.file.fileSize, chunks: data.file.chunks?.length || 0 }, sourceClientId);
...
async function handleVirtualMediaRequest(request, url, targetClientId = null) {
  ...
  postDebug("sw", "range:start", { url: url.pathname, start: range.start, end: range.end, fileSize: file.fileSize }, targetClientId);
...
function makeRangeStream(file, start, end, signal, targetClientId = null) {
  ...
  return new ReadableStream({
    async pull(controller) {
      ...
      debug("slice:start", { cursorStart: cursor, cursorEnd: globalEnd, localStart, localEnd, sliceSize: globalEnd - cursor + 1 });
      ...
      try {
        ...
        if (!resp.ok && resp.status !== 206) {
          debug("slice:response-error", { status: resp.status });
          errorStream(controller, new Error(`Backing chunk fetch failed: HTTP ${resp.status}`));
          return;
        }
        debug("slice:response", { status: resp.status });
        ...
        debug("slice:done", { bytes: bytes.byteLength, durationMs: Math.round(performance.now() - startedAt) });
        cursor = globalEnd + 1;
      } catch (err) {
        if (isAbortError(err) || isClosedStreamError(err)) {
          debug("stream:abort", { cursor });
    ...
    cancel() { markStreamClosed(); debug("stream:cancel", {}); },
  });
}
